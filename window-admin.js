...
<p class="status quiet">还没有验真小票。正常聊一轮后，后台会自动记录；不用给小克发送检查指令。</p></details>`;
  ...
  const guarded = receipt.status === "guarded" || receipt.stopSequenceBlocked === true ||
    receipt.stopReason === "stop_sequence";
  const failed = !guarded && (receipt.status === "empty-result" ||
    receipt.status === "upstream-error" || receipt.isError === true);
  const tone = guarded ? "fresh"
    : failed || hasUpstreamModel && !modelMatches ? "warm"
    : receipt.signatureSeen ? "fresh" : "quiet";
  const headline = guarded
    ? "已在 API 层拦截异常续写；截断前的安全正文（如有）已正常交付。"
    : failed
...
<div class="grid verify-grid">
  ...
  ${guarded ? `<div class="tile">本轮状态<strong>异常续写已拦截</strong><small>没有把伪造 user 段交付给前端</small></div>
  <div class="tile">安全正文<strong>${receipt.safeTextAvailable === false ? "本轮没有" : "已按实际结果处理"}</strong><small>显示提示不会进入原生上下文</small></div>` : failed ? `<div class="tile">本轮状态<strong>${receipt.emptyResult ? "零 token 空回" : "上游错误"}</strong><small>${receipt.apiErrorStatus ? `HTTP ${escapeHtml(receipt.apiErrorStatus)}` : "未取得 HTTP 状态"}</small></div>
