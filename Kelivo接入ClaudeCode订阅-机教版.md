...
| `BROWSER_MCP_TOKEN` | (可选)云端浏览器的访问令牌 | 作为 `X-Token` 请求头注入 MCP；只放 Zeabur 环境变量，绝不提交仓库或写进日志 |
...
3. 默认行为:新加坡时间06:00-24:00内,距上一轮对话(含唤醒轮)约50-60分钟 → AI 收到【自主时间】提示 → **它自己决定**找你、整理记忆、逛花园、调用啵啵鸟或做别的小事，也可以只回【沉默】。这一轮会尝试刷新1小时提示词缓存
4. 手机打开 `https://你的域名/admin/wake`,用 `SHIM_KEY` 登录,可在“白天06:00-24:00”和“全天24小时”之间热切换。立即生效,不会重启 Claude 会话;选择保存在私人磁盘,部署后不丢
