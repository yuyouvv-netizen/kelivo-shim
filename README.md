...
- 标准 200K 长对话在真实压缩线的 90% 自动写一封 OB Letter 续接信;压缩后自动取回全部钉选桶、按 OB 排名浮现的 8 个普通桶与最近三天的续接信
...
- 自主唤醒默认只在新加坡时间 06:00-24:00、空闲约 50-60 分钟后复用已恢复历史的常驻进程;小克可自行找人、整理记忆或调用花园/啵啵鸟等工具,也可保持沉默;手机 `/admin/wake` 可热切换全天模式
- 手机打开 `/admin/session` 并用 `SHIM_KEY` 登录,可不经归档主动放下碎片化原生会话;后端保持空白直到新 4.6 对话的第一条真实消息
- 手机打开 `/admin/window` 可只读查看当前 K 数、压缩状态和模型/思考验真小票，也可热修改 Bark 通知名字;进度读取与改名都不会触发 Claude 轮次，85% 提醒与 90% 续接信回执按原生会话持久化，部署重启不会重复触发
...
| `window-admin.js` | 手机 `/admin/window` 窗口进度、验真与 Bark 名字页,写操作使用 `SHIM_KEY`、CSRF 和安全 Cookie |
