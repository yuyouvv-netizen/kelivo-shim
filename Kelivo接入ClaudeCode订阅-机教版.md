...
function wakeTick(force) {
  if (!inSingaporeWakeWindow()) return;
  if (busy || queue.length) return;
  ...
  enqueue({ text: `【系统·自主时间】现在新加坡时间 ${now},对方已约 ${idleUserMin} 分钟没有消息${sinceSpoke}。这轮是留给你自己的:${speakLine}没什么想说的就只回【沉默】两个字,这轮只用来保持你的状态和记忆连续。`, images: [], system: spawnedSystem, sse: sink, newWindow: false });
}
...
| `WAKE_IDLE_MIN` | `50` | 自主时间空闲阈值;`WAKE_CHECK_MIN` 为本地检查频率(默认10),实际约50-60分钟触发。默认新加坡时间08:00-24:00,可在手机 `/admin/wake` 热切换全天模式 |
...
3. 默认行为:新加坡时间08:00-24:00内,距上一轮对话(含唤醒轮)约50-60分钟 → AI 收到【自主时间】提示 → **它自己决定**发条通知(弹你锁屏)或只回【沉默】。两条路都会尝试刷新1小时提示词缓存;发言频率也交给 AI 自己把握(提示里会告知距上次开口多久)
4. 手机打开 `https://你的域名/admin/wake`,用 `SHIM_KEY` 登录,可在“白天08:00-24:00”和“全天24小时”之间热切换。立即生效,不会重启 Claude 会话;选择保存在私人磁盘,部署后不丢
5. 成本:每次自主轮都是一次真实 Claude 调用;全天模式夜间也会继续产生调用
6. 测试:`curl -X POST "https://<shim域名>/hb?key=<SHIM_KEY>"` 强制触发一次
